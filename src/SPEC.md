...
| 2026-09-25 | **Other catalog tasks** (kit-style kinematics; `src/task_train.py`, plan in `informe/plan_otras_tareas.md`). Regression gate: `pursuit_kit` through the generic trainer | all 20 greedy evaluations of `runs_kit_final` seed 0 reproduced bit-for-bit | `src/tests/test_task_train.py`, scratch check |
| 2026-09-25 | **Shooting**, calibration (rule fixed before training) | provisional σ 4°, reach 2 m, p_out 0.1 failed "CONDUCIR optimal somewhere"; grid rule selected p_out 0.02. CONDUCIR optimal in 1 of 36 states (margin 0.25) | `python -m src.shooting_optimal --calibrate …`, `shooting_calibration.json` |
| 2026-09-25 | **Shooting, exact optimum** (normal-CDF outcome probabilities; true state vs our 36 states) | open goal 100%; with keeper: true optimum 95.2%, representation optimum 93.3% (sampling std of one evaluation 1.1 pp) | `shooting_optimal.json` |
| 2026-09-25 | **Shooting, 4 methods × 2 schedules**, 50k episodes, 5 seeds, α 0.03 (decay-only sweep, seeds 5–9) | every condition meets both criteria in every seed; keeper: Q-learning 92.3% [91.8, 92.6] decay vs 87.5% const; MC sample-average best under const ε (93.3%) | `runs_shooting`, `shooting_summary.json`, notebook 10 |
| 2026-09-25 | Shooting diagnostics (const ε) | const-ε gap closed by α 0.1, Q₀ = 100 or 4× budget → slow correction from Q₀ = 0, not a different solution; max_a Q − V* ≈ −6.6 at α 0.03 | `runs_shooting_diag`, notebook 10 §7 |
| 2026-09-26 | **Dribbling**, constructive bound | scripted controller 20 000/20 000 successes, 60.4 mean steps, max 85 < 100 | `dribbling_scripted.json`, notebook 11 §1 |
| 2026-09-26 | **Dribbling, 4 methods × 2 schedules**, 60k episodes, α per (method, schedule) | Q-learning last-5 mean 97.7% [96.3, 99.0] decay vs 89.7% const; SARSA 94.3 / 96.8%; MC sample-average 90.4 / 70.6%; MC constant α ≤ 30% | `runs_dribbling`, `dribbling_summary.json`, notebook 11 |
| 2026-09-26 | Dribbling diagnostics | greedy oscillation = TURN ± two-cycles between aggregated states (near-tied Q); 245 states: Q-learning and SARSA 100% in every last-5 evaluation (36 states: 95.1 / 97.6%); losses while exploring ≤ 0.5%, loss radius 8 m changes nothing | `runs_dribbling_diag`, notebook 11 §8 |
| 2026-09-26 | **2v1 passing**, design fix before training | GIRAR flipped sides every step with the defender straight behind (trap at the zone edge); now turns toward the zone centre in that case | `src/passing_env.py`, `test_passing_env.py` |
| 2026-09-26 | **2v1, 4 methods × 2 schedules**, 60k episodes, α 0.03 (per-schedule sweep) | success (possession > 50 steps and ≥ 3 passes), last-5 mean: Q-learning 64.8% [61.3, 68.1] decay / 65.5% const; SARSA 54.2%; MC 7.8–26.1%; heuristic 10.8% | `runs_passing`, `passing_summary.json`, `passing_references.json`, notebook 12 |
| 2026-09-26 | 2v1 diagnostics | SARSA tackled more than Q-learning while exploring (13.8 vs 8.7% at p 0.5), gap not growing with p ∈ {0.2, 0.5, 0.8}; 1 944-state representation worse at 2× budget (Q 62.4 vs 67.9%) | `runs_passing_diag`, notebook 12 §8 |
| 2026-09-26 | **Live execution checks** (greedy Q-learning, state from trainer ground truth; one server session per run) | 0 missed cycles in all runs. Shooting 20/20 open, 20/20 with an idle keeper client, measured angular error std 1.75° (model 4°). Dribbling 10/10, literal `kick 25` free travel 13.6 m mean (44 kicks; model 2 m). 2v1 0/10 (7 out, 3 defender ball; passes take several cycles) | `python -m src.live_tasks …`, `live_shooting.json`, `live_dribbling.json`, `live_passing.json` |
